...
class ViewControllerCrearCuenta: UIViewController , FBSDKLoginButtonDelegate{
    ...
    override func viewDidLoad() {
        //Valida si el usuario ya a ingresado con fb
        /*
    ...
    */
    ...
    @objc func accesoPorCorreo(correo:String,contrasena:String) {
        Auth.auth().createUser(withEmail: correo , password: contrasena) { (user, error) in
           if let error = error{
    ...
    */
    func creaUI() {
        ...
        let buttonText = NSAttributedString(string: "Crea una cuenta con Facebook")
        loginButtonFb.setAttributedTitle(buttonText, for: .normal)
        let origen = CGPoint(x: btnCrearCuenta.frame.minX, y: btnCrearCuenta.frame.maxY + 60)
        let tamano = CGSize(width: btnCrearCuenta.frame.width, height: btnCrearCuenta.frame.height)
        loginButtonFb.frame = CGRect(origin:origen, size: tamano)
        loginButtonFb.center = CGPoint(x: view.frame.midX, y: btnCrearCuenta.frame.maxY + 50)
    ...
    //*****************************************************************************************************
    //Metodo del delegado de Facebook
